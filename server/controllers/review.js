  ...
  Booking.findById(bookingId)
    ...
    .exec(async (err, foundBooking) => {
      ...
      if (rental.user.id === user.id) {
        return res.status(400).send({errors: [{title: 'Invalid user!', detail: 'Cannot create review on your rental'}]});
      }
      const foundBookingUserId = foundBooking.user.id;
      if (foundBookingUserId !== user.id) {
        return res.status(400).send({errors: [{title: 'Invalid user!', detail: 'Cannot create review on someone else booking'}]});
      }
      ...
      if (!endAt.isBefore(timeNow)) {
        return res.status(400).send({errors: [{title: 'Invalid date!', detail: 'You can place review only after your trip is finished'}]});
      }

      if (foundBooking.review) {
        return res.status(400).send({errors: [{title: 'Booking error!', detail: 'Only one review per booking is allowed'}]});
      }
